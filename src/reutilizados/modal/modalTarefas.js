...
const ModalTarefas = ({ tarefaSelecionada, fechaModal }) => {
  const [nAndamento, setNAndamento] = useState(tarefaSelecionada.andamento);
  const [nAtribuido, setNAtribuido] = useState(tarefaSelecionada.atribuido);
  const [nCriadoEm, setNCriadoEm] = useState(tarefaSelecionada.criado_em);
  const [nDescricao, setNDescricao] = useState(tarefaSelecionada.descricao);
  const [nEsforcoEstimado, setNEsforcoEstimado] = useState(tarefaSelecionada.esforco_estimado);
  const [nEsforcoReal, setNEsforcoReal] = useState(tarefaSelecionada.esforco_real);
  const [nInicioEstimado, setNInicioEstimado] = useState(tarefaSelecionada.inicio_estimado);
  const [nInicioReal, setNInicioReal] = useState(tarefaSelecionada.inicio_real);
  const [nSolicitante, setNSolicitante] = useState(tarefaSelecionada.solicitante);
  const [nTags, setNTags] = useState(tarefaSelecionada.tags);
  const [nTerminoEstimado, setNTerminoEstimado] = useState(tarefaSelecionada.termino_estimado);
  const [nTerminoReal, setNTerminoReal] = useState(tarefaSelecionada.termino_real);
  const [nTitulo, setNTitulo] = useState(tarefaSelecionada.titulo);
    ...
    <div class="siimple-modal siimple-modal--medium" id="modal">
      <div class="siimple-modal-content">
        <div class="siimple-modal-header">
          <div class="siimple-modal-header-title">{tarefaSelecionada.titulo}</div>
        ...
        <div class="siimple-modal-body">
          ...
          <input type="text" class="siimple-input" style={{ width: '100%' }} value={nDescricao} onChange={ev => novaDescricao(ev)} />
